...
func TestReserveBookWithZeroAvailability(t *testing.T) {
	...
	assert.Contains(t, rr.Body.String(), config.NoReserve)
}

func TestReserveBookWithWrongID(t *testing.T) {
	req, _ := http.NewRequest("GET", "/config/reserve/books/22", nil)
	rr, _ := commonResponseModifyBook(t, req)
...
func TestReleaseBookWithFullBooked(t *testing.T) {
	...
	assert.Contains(t, rr.Body.String(), config.NoRelease)
}

func TestReleaseBookWithWrongID(t *testing.T) {
	req, _ := http.NewRequest("GET", "/config/release/books/44", nil)
	rr, _ := commonResponseModifyBook(t, req)
...
func TestUpdateBookWithSuccess(t *testing.T) {
	var jsonStr = []byte(`{   
    "isbn": "ISBN05",
    "Price": 6999,
...
func TestUpdateBookWithError(t *testing.T) {
	...
	req, _ := http.NewRequest("PUT", "/config/update/books/55", bytes.NewBuffer(jsonStr))
	rr, _ := commonResponseModifyBook(t, req)
